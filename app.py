...
def parse_table(html, table_class):
    ...
    for row in tbody.find_all("tr"):
        ...
        # Фильтр 1: пропускаем прогнозы с результатом (Win/Loss/Void)
        result_cell = row.find("td", class_="result")
        if result_cell:
            result_text = result_cell.get_text(strip=True).lower()
            if any(x in result_text for x in ("win", "loss", "void", "push")):
                continue
        ...
        # Фильтр 2: пропускаем закрытые прогнозы (Unlock)
        if "unlock" in signal.lower() or "unlock" in odd.lower():
            continue
