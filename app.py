...
def check_once(seen, first_run):
    ...
    total = len(all_bets)
    filtered = {"result": 0, "unlock": 0, "duplicate": 0, "first_run": 0}
    published = 0

    print(f"=== Найдено прогнозов: {total} ===", flush=True)

    for bet in all_bets:
        # Фильтр 1: результат (Win/Loss/Void) — только если ячейка НАЧИНАЕТСЯ со слова
        if bet["result"]:
            r = bet["result"].strip().lower()
            if r.startswith(("win", "loss", "void", "push", "half win", "half loss")):
                filtered["result"] += 1
                print(f"  [отсеян: result='{bet['result']}'] {bet['league']} | {bet['match']} | {bet['signal']}", flush=True)
                continue
            else:
                # Если в ячейке есть текст, но он не является результатом — покажем, чтобы понять
                print(f"  [result есть, но не Win/Loss] '{bet['result']}' | {bet['match']}", flush=True)

        # Фильтр 2: Unlock
        if "unlock" in bet["signal"].lower() or "unlock" in bet["odd"].lower():
            filtered["unlock"] += 1
            print(f"  [отсеян: unlock] {bet['league']} | {bet['match']}", flush=True)
            continue
        ...
        if first_run:
            ...
            print(f"  [калибровка] {bet['match']} | {bet['signal']}", flush=True)
            continue

        # Публикация
        print(f"  ✅ публикую: {bet['league']} | {bet['match']} | {bet['signal']} | {bet['odd']}", flush=True)
        try:
    ...
    save_seen(seen)
    print(f"=== Итог: опубликовано {published} | отсеяно — result:{filtered['result']} unlock:{filtered['unlock']} дубликатов:{filtered['duplicate']} калибровка:{filtered['first_run']} ===", flush=True)
    return published
